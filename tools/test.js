...
const containsKnownFailure = (failure) => {
  if (knownFailures.includes(failure)) {
    return true
  }
  const hooks = ['"before all" hook', '"before each" hook']
  for (let i = 0; i < hooks.length; i++) {
    const hook = hooks[i]
    const parts = failure.split(hook)
    if (parts.length > 1) {
      if (knownFailures.includes(parts[0] + hook)) {
        return true
      }
    }
  }
...
child.on('close', function (code) {
  ...
  const unknownFailures = failures.filter((failure) => !containsKnownFailure(failure))
