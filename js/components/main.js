...
const shieldUtil = require('../../app/common/lib/shieldUtil')
...
class Main extends ImmutableComponent {
  ...
  componentDidMount () {
    ...
    ipc.on(messages.OPEN_BRAVERY_PANEL, (e) => {
      if (!shieldUtil.braveShieldsDisabled(self.props.windowState)) {
        this.onBraveMenu()
      } else {
  ...
  onBraveMenu () {
    if (!shieldUtil.braveShieldsDisabled(this.props.windowState)) {
  ...
  get activeSiteSettings () {
    return this.frameSiteSettings(frameStateUtil.activeRequestedLocation(this.props.windowState))
  }
  ...
  render () {
    ...
    const braveShieldsDisabled = shieldUtil.braveShieldsDisabled(this.props.windowState)
    ...
    const activeRequestedLocation = frameStateUtil.activeRequestedLocation(this.props.windowState)
          ...
          ref={(node) => { this.tabContainer = node }}>
          {
            sortedFrames.map((frame) =>
              <Frame
                ...
                onFindHide={this.onFindHide}
                enableNoScript={siteUtil.enableNoScript(this.frameSiteSettings(frame.get('location')), this.props.appState)}
